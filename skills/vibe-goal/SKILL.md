...
2. **Standards Axis.** Run `/vibe-review` from Stage 2 fixed point to recorded integration branch and commit. Per-ticket reviews saw single slices; this review examines the space between them, where interesting findings reside. If `/rq` will run in step 3, tell it Spec is handled by the gate and skip that axis. If `/rq` is unavailable, run both axes — do not skip Spec.
...
4. **Adjudicate.** Present Standards findings and, if a gate actually ran, gate reports side by side without merging. Items marked `not satisfied` or `unknown` by a gate that ran, plus Standards findings user wants resolved, become **remediations**: route back to planning skills to design and publish new tickets, which flow through Stage 3 — never patch inline directly. Remediation tickets are genuinely new canonical tickets with own IDs and assigned workspaces, dispatched and replayed like any other ticket. After those tickets integrate, re-run the gate if it ran, otherwise re-run `/vibe-review`; goals never close on `FAIL`.
...
7. **Report.** Goal, landed tickets with links, integration branch and reviewed commit, final landing proof, overall gate status if a gate ran (otherwise that it was skipped), Standards findings, and intentional open items.
