...
- **Blocking**: GitLab's **native blocking links** — canonical UI-rendered representation. Add via `/blocked_by #<n>` quick action posted as a note (`glab issue note <child> --message "/blocked_by #<blocker>"`). Native blocking links require Premium/Ultimate. On free tiers (or when unavailable), fall back to `Blocked by: #<n>, #<n>` lines at top of description. A ticket unblocks when all blocking issues close.
- **Frontier Query**: Fetch `glab issue list -F json` restricted to map children, excluding those with open blockers (native `blocked_by` links to open issues via `glab api projects/:id/issues/:iid/links` or open issues in `Blocked by` line) or assignees, ordered by map position.
