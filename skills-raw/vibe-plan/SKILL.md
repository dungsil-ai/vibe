...
Stage 2로 어떻게 진입했든, 명세를 쓰기 전에 확정된 결정을 `/vibe-modeling` 기준으로 확인한다. 이미 끝난 결정을 다시 인터뷰하지 않는다. 새 도메인 용어는 `CONTEXT.md`에 기록하고, ADR 세 조건을 만족하지만 아직 기록되지 않은 결정은 ADR로 쓴다. 같은 결정을 다시 기록하지 않는다.

호스트형 트래커에 명세, 라벨, 티켓을 처음 게시하거나 수정하기 전에, 이번 계획에서 바꾼 `CONTEXT.md`와 ADR을 커밋한다. `git add .`을 쓰지 않고 이번 실행의 정확한 경로만 stage하며, 기존 사용자 변경을 섞지 않는다. staged diff를 확인하고 commit SHA를 검증한다. 안전하게 분리할 수 없는 기존 변경이나 커밋 실패가 있으면 멈추고 원격 트래커를 바꾸지 않는다. push는 별도 요청이 있을 때만 한다. 로컬 모델링 문서가 바뀌지 않았으면 커밋을 만들지 않는다.
