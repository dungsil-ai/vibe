...
**결정 지도는 요청이 아니다.** `상태:초안` 라벨이 붙은 이슈, 또는 그 `유형:` 라벨 결정 티켓은 `/vibe-deep-plan`에 속한다 — 진행 중인 계획 산물이지 트리아지 대상이 아니다. 사용자가 그것을 가리키면 그렇게 말하고 대신 `/vibe-deep-plan`을 제안한다. 그런 지도가 정리되면, 그 Destination과 Decisions-so-far는 **Stage 2 — 명세**의 유효한 입력이다.
