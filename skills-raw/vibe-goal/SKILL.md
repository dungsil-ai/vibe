...
`/vibe-review`는 읽기 전용이다: 발견사항을 보고할 뿐 트래커 상태를 절대 바꾸지 않는다. 리뷰 통과, 서브에이전트 완료 주장, 또는 사용자의 일반적인 목표 요청은 트래커 쓰기를 권한 부여하지 않는다. 부모 spec 이슈는 읽기 전용이다: 트래커 변경 미리보기에 절대 포함하지 않고 수정하지도 않는다.

목표는 통합과 최종 반영을 책임진다. **호스티드** 트래커의 경우, 작업이 **통합 작업공간**에만 존재하는 동안에는 티켓 상태를 미리보기하거나 바꾸지 않는다; Stage 4에서 정확한 리뷰된 통합 commit SHA가 원본 대상 branch에 있다는 권위 있는 증거가 나온 후에만 모든 체크박스, 전체 코멘트, 각 상태 또는 닫기 변경의 정확한 미리보기를 보여주고, 쓰기 직전에 별도의 명시적 승인을 기다린다 — 거절이나 무응답은 트래커를 변경하지 않은 채로 둔다. **버전 관리되는 local Markdown** 트래커의 경우 미리보기할 것이 없다: 각 티켓의 체크박스는 그 티켓 자신의 구현 commit에 실려 있으므로, 통합과 반영이 code와 checklist를 함께 옮긴다. 어느 쪽이든 오케스트레이터는 티켓 파일을 직접 수정하지 않고, 대상 branch에 트래커 전용 commit을 올리지 않는다.
...
상태는 `pending` → `running` → `integrated` → `landed`, 또는 `blocked` / `failed`다. `integrated`는 리뷰를 거친 티켓 결과가 통합 head에 재생되고 그 head에서 티켓 귀속 검증을 통과했음을 의미한다; 의존 티켓의 블록을 푼다. `landed`는 Stage 4가 리뷰된 통합 결과가 원본 대상 branch에 있음을 증명한 후에만 기록된다. 어느 상태도 호스티드 트래커 쓰기를 권한 부여하지 않는다.
...
2. **Standards 축.** Stage 2 고정점부터 기록된 통합 branch와 commit까지 `/vibe-review`를 실행한다. 티켓별 리뷰는 각자 한 slice만 봤다; 이 리뷰는 그 사이를 보며, 흥미로운 발견사항은 거기에 있다. Spec 축은 게이트가 처리하므로 중복하지 않는다고 알려준다.
3. **Spec 판정.** Stage 2 고정점을 change 경계로, 기록된 통합 결과를 head로 `/rq`를 실행하고, 구현 도메인(`CODE`, 목표가 필요하면 `MIGRATION` 추가)으로 범위를 좁힌다. 소스 요구사항은 목표의 **완료 정의**이다 — 실행이 실제로 약속한 인수 조건(spec의 인수 조건, 또는 spec에 없으면 원장의 목표 줄). 그 너머의 user stories는 추적 행으로 남으며, 범위 밖이다; 큰 spec의 모든 story를 게이팅하면 티켓별 리뷰가 이미 다룬 것을 중복한다. 게이트는 항목별 상태와 집계된 `PASS` / `WARNING` / `NEEDS_REVIEW` / `FAIL`을 반환한다.
   - 기본으로 게이트의 `LIGHT` tier를 쓴다. 목표가 항상 무거운 tier가 필요한 도메인 — security, auth, permissions, persistence, transactions, external integrations — 을 건드렸을 때만 `HEAVY`를 쓴다 — `vibe-review`가 에스컬레이션하는 같은 규칙.
   - Operation, deployment, 데이터 의무는 **별도 게이트**로 둔다. 구현 판정을 낮추지 않으며, 사용자가 요청하지 않으면 여기서 실행하지 않는다.
4. **판정하기.** Standards 발견사항과 게이트 보고를 나란히, 병합 없이 보여준다. 게이트가 `not satisfied` 또는 `unknown`으로 표시한 것, 그리고 사용자가 고치기 원하는 Standards 발견사항은 **수정**이 된다: 계획 스킬로 다시 라우팅하고, 그 스킬이 새 티켓을 설계하고 발행한다; 그런 다음 Stage 3를 통해 다시 흘러간다 — 직접 inline patch하지 않는다. 수정 티켓은 자체 정규 ID와 자체 할당된 작업공간을 가진 진짜 새 정규 티켓이며, 다른 티켓처럼 파견되고 재생된다. 그 티켓이 통합된 후 게이트를 다시 실행한다; `FAIL`에서 목표가 닫히지 않는다.
...
6. **반영을 증명한 뒤, 승인을 구한다.** 권위 있는 증거가 정확한 리뷰된 통합 commit SHA가 원본 대상 branch에 있음을 증명한 후에만, 영향받은 티켓을 `landed`로 기록한다. 버전 관리되는 local Markdown 트래커의 경우 반영이 이미 각 티켓의 체크된 checklist를 실었으므로 미리보기할 것도 쓸 것도 없다 — 대상에 트래커 전용 commit을 절대 추가하지 않는다. 호스티드 트래커의 경우, 각 티켓에 대해 제안된 정확한 체크박스 변경, 전체 코멘트, 상태 또는 닫기 변경을 보여준다; 부모 spec 이슈는 제외한다. 그 미리보기만 적용하기 전에 별도의 명시적 승인을 기다린다. 거절이나 무응답은 트래커를 변경하지 않은 채로 둔다.
7. **보고.** 목표, 링크와 함께 반영된 티켓, 통합 branch와 리뷰된 commit, 최종 반영 증거, 게이트의 전체 상태, Standards 발견사항, 그리고 의도적으로 남긴 미완료 사항.
