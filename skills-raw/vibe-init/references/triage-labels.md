...
`상태:초안`은 트리아지 상태와 같은 축에 있으므로 트리아지 상태를 배제한다. 결정 맵은 진행 중인 계획 산출물이지 평가를 기다리는 요청이 아니며, 트리아지 대상도 아니다. 같은 이유로 맵의 결정 티켓은 `유형:` 라벨만 가진다. 맵이 통과하고 `vibe-plan`이 구현 티켓을 발행한 뒤에야 트리아지 상태가 다시 적용된다. 스킬은 정규 역할로 말하고, 위 표의 오른쪽 칸이 유일한 라벨 어휘다. 그 문자열을 영어(`status:draft`, `type:research` 등)로 옮기지 않는다.
